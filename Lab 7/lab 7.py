...
class Bank:
    ...
    def search_seller(self, sellers):
        for seller in self.__seller_list:
            if seller.name == sellers:
                return seller
        return None
...
#     def __str__(self):
#         if self.__target_account is None:
#             return f"{self.__transaction_type} {self.__amount} {self.__total}"
#         return f"{self.__transaction_type} {self.__amount} {self.__total} {self.__target_account.account_no}"
...
class Account:
    ...
    def deposit(self,money):
        self.__amount = self + money
        
    def withdraw(self, money):
        self.__amount = self - money

    def transfer(self, strng, amount, target_account):
        target_account.amount = target_account.amount + amount
        self.__amount = self.__amount - amount
        self.add_transaction(Transaction("Transfer", - amount, self.__amount, target_account))
        target_account.add_transaction(Transaction("Transfer", amount, target_account.amount, None))
...
class Transaction:
    ...
    def __str__(self):
        if self.__target_account is None:
            return f"{self.__transaction_type} Amount: {self.__amount}, Total: {self.__total}"
        return f"{self.__transaction_type} Amount: {self.__amount} to {self.__target_account.account_no}, Total: {self.__total}"
...
class ATM_Card(Card):
    def __init__(self, card_no, account, pin):
        Card.__init__(self, card_no, account, pin)
        self.__fee = 150
...
class ATM_machine:
    ...
    def deposit(self, account, amount):
        ...
        account.add_transaction(Transaction("Deposit", amount, account.amount, None))

    def withdraw(self, account, amount):
        account.withdraw(amount)
        account.add_transaction(Transaction("Withdraw", - amount, account.amount, None))
...
class Seller:
    ...
    def paid(self, account, amount, target_account):
        account.transfer("000", amount, target_account)
...
class EDC_machine:
    ...
    def paid(self, card, amount, target_account):
        if isinstance(card,ATM_Card):
            account = scb.search_account_from_card(card.card_no)
            account.transfer("000",amount,target_account)
        else:
            card.account.transfer("000",amount,target_account)
